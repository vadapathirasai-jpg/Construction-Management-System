...
const definitions = {
  ...
  materials: {
    ...
    fields: [["name", "Material Name"], ["category", "Category"], ["quantity", "Quantity", "number"], ["unit", "Unit"], ["supplier", "Supplier"], ["cost", "Cost", "number"], ["status", "Stock Status", "select", ["In Stock", "Low Stock", "Out of Stock"]]],
  },
  expenses: {
...
function useFilteredRows(rows, filterKey) {
  ...
  const filtered = useMemo(() => rows
    ...
    .filter((row) => !secondaryFilter || row.project === secondaryFilter)
    .sort((a, b) => {
...
function RecordModal({ type, mode, record, onClose, onSave, saving = false, saveError = "" }) {
  ...
  const submit = async () => {
    const nextErrors = {};
    config.fields.forEach(([key, label]) => { if (values[key] === undefined || values[key] === "") nextErrors[key] = `${label} is required`; });
  ...
  const fields = config.fields.map(([key, label, typeName = "text", options]) => {
    if (key === "project" && type !== "projects") {
      return [key, label, "select", data.projects.map((p) => p.name)];
    }
    if (key === "manager") {
      ...
      const allManagers = Array.from(new Set([
        ...managers,
        ...(data.currentUser?.role === "Project Manager" ? [data.currentUser.name] : []),
        "Arun Kumar", "Meera Shah", "Vikram Singh", "Priya Nair"
      ]));
      return [key, label, "select", allManagers];
    }
    return [key, label, typeName, options];
  });

  return <Modal title={`${mode === "add" ? "Add" : mode === "edit" ? "Edit" : "View"} ${config.title.slice(0, -1)}`} description={isView ? "Record details" : "Complete all required fields."} onClose={onClose} footer={!isView ? <><Button variant="secondary" onClick={onClose} disabled={saving}>Cancel</Button><Button onClick={submit} disabled={saving}>{saving ? "Saving..." : "Save Record"}</Button></> : <Button variant="secondary" onClick={onClose}>Close</Button>}>
    {saveError && <div className="mb-5 rounded-md border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700">{saveError}</div>}
    <div className="grid gap-5 sm:grid-cols-2">
      {fields.map(([key, label, typeName = "text", options]) => <Field key={key} label={label} error={errors[key]}>
        {typeName === "select" ? <select disabled={isView} className="form-control disabled:bg-slate-50" value={values[key] || ""} onChange={(event) => setValues({ ...values, [key]: event.target.value })}><option value="">Select {label.toLowerCase()}</option>{options && options.map((option) => <option key={option} value={option}>{option}</option>)}</select>
...
function PageShell({ type, columns, renderRow, extraFilters, summary }) {
  ...
  const assignedNames = data.accessibleProjects.map((project) => project.name);
  const rows = type === "projects" ? data.accessibleProjects
    : type === "workers" && data.currentUser.role !== "Admin" ? data.workers.filter((worker) => assignedNames.includes(worker.project))
      : type === "expenses" && data.currentUser.role === "Project Manager" ? data.expenses.filter((expense) => assignedNames.includes(expense.project))
        : data[type];
  const config = definitions[type];
...
export function Workers() {
  return <PageShell type="workers" columns={[{ label: "Name", key: "name" }, { label: "Role", key: "role" }, "Phone", { label: "Daily Wage", key: "wage" }, { label: "Assigned Project", key: "project" }, { label: "Status", key: "status" }, "Actions"]} renderRow={(w, actions) => <tr key={w.id}><td className="px-5 py-4"><p className="font-medium">{w.name}</p><p className="mt-1 text-xs text-slate-400">{w.id}</p></td><td className="px-5 py-4 text-slate-600">{w.role}</td><td className="whitespace-nowrap px-5 py-4 text-slate-600">{w.phone}</td><td className="px-5 py-4 font-medium">{formatCurrency(w.wage)}</td><td className="px-5 py-4 text-slate-600">{w.project}</td><td className="px-5 py-4"><StatusBadge status={w.status} /></td><td className="px-5 py-4">{actions}</td></tr>} />;
}
...
export function Expenses() {
  ...
  const visibleExpenses = currentUser?.role === "Project Manager" ? expenses.filter((expense) => projectNames.includes(expense.project)) : expenses;
